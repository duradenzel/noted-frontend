...
const useCampaignLogic = (campaign: Campaign) => {
  ...
  const handleEditCampaign = useCallback(async () => {
      ...
      try {
        await axios.put<Campaign>(`http://localhost:5170/campaigns/${campaign.campaignId}`, editedCampaign);
  ...
  const handleDeleteCampaign = useCallback(async () => {
    try {
      await axios.delete<Campaign>(`http://localhost:5170/campaigns/${campaign.campaignId}`);
...
const CampaignHeader: React.FC<HeaderProps> = ({ campaign }) => {
  const { editedCampaign, handleEditCampaign, handleDeleteCampaign, handleInputChange } = useCampaignLogic(campaign);
      ...
      <div className="w-full mb-4 md:mb-0 md:mr-4">
        ...
        <div className="header-content w-full">
          <div className='flex justify-between items-start w-full'>
            ...
            />
            {isChanged && (
              <div className='bg-primary-500 text-white rounded h-6 w-6 items-center  flex justify-center' onClick={handleEditCampaign}>
